...
// 16.1 => using trim, string cases, remove
// below function is from assignment
...
const normalizeAuthorName = function (author) {
  if (author.lastIndexOf('(Contributor)') !== -1) {
    author = author.slice(0, author.lastIndexOf('(Contributor)') - 1).trim();
  }
  const authorArr = [];
  author.split(' ').forEach(auth => {
    authorArr.push(auth[0].toUpperCase() + auth.slice(1).toLowerCase());
  });
  return authorArr.join(' ');
};
console.log(normalizeAuthorName('  JuliE sussMan (Contributor)'));
console.log(normalizeAuthorName('Jennifer'));

// 16.2 => replace a word
const newBookTitle = books[1].title.replace('Programs', 'Software');
console.log(newBookTitle);

// 16.3 => includes, startsWith, endsWith
